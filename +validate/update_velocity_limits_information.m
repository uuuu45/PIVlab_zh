...
if ~isempty (velrect) || ~isempty(velrect_freehand)
	...
	if state==1
		set (handles.vel_limit_active, 'String', '矩形限制已启用', 'backgroundcolor', bg_col);
	elseif state == 2
		set (handles.vel_limit_active, 'String', '手绘限制已启用', 'backgroundcolor', bg_col);
	elseif state == 3
		set (handles.vel_limit_active, 'String', '矩形与手绘限制已启用', 'backgroundcolor', bg_col);
	elseif state == 0
		if gui.retr('darkmode')
		...
		set (handles.vel_limit_active, 'String', '未启用限制', 'backgroundcolor', bg_col);
	end
	if ~isempty (velrect)
		umin=velrect(1);
		umax=velrect(3)+umin;
