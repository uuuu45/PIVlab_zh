...
try
    ...
    gui.custom_msgbox('error',getappdata(0,'hgui'),'错误','Error: Image Acquisition Toolbox not available! This camera needs the image acquisition toolbox.','modal');
...
try
    ...
    gui.custom_msgbox('error',getappdata(0,'hgui'),'错误','Error: Camera not found! Is it connected?','modal');
end
...
if strcmpi(event.Data.MessageID,'imaq:imaqmex:outofmemory')
	...
	gui.custom_msgbox('error',getappdata(0,'hgui'),'错误','Image capture timeout. Most likely, memory is full and you need to lower the amount of frames to capture to fix this error. It is also possible that the synchronization cable is not plugged in correctly.','modal');
end