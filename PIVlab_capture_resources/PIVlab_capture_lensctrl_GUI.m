...
do_calib=0;
answer = gui.custom_msgbox('quest',getappdata(0,'hgui'),'水平校准',{'请使用精密水平仪调平相机，然后按"开始"。' ' ' '传感器已在工厂校准，通常不需要手动校准。'},'modal',{'开始','取消'},'开始');
switch answer
	case '开始'
		do_calib=1;
	case '取消'
...
if do_calib==1
	...
	if alreadyconnected==1
		...
		Roll=zeros(1,10);
		Pitch=zeros(1,10);
		set(handles.calibrate,'String','请稍候...')
		...
		if (stdRoll + stdPitch) > 0.1
            gui.custom_msgbox('error',getappdata(0,'hgui'),'错误','Camera was shaking too much!','modal');
		else
