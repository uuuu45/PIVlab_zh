...
if isempty(resultslist)==0
	if size(filepath,1)>0
		...
		if selectionok==0
			gui.custom_msgbox('error',getappdata(0,'hgui'),'错误',['帧选择语法错误。请使用以下语法（示例）：' sprintf('\n') '1:3' sprintf('\n') '1,3,7,9' sprintf('\n') '1:3,7,8,9,11:13' sprintf('\n') '如需多次平均（如相位平均），请使用行向量：[1:10:end;2:10:end]'],'modal');
		end
		if selectionok==1
			...
			if sizeerror==0
					...
					for i=1:size(strnum,2)
						if size(resultslist,2)>=strnum(i) %dann ok
							...
							if isempty(x_tmp)==1 %dann nicht ok
								gui.custom_msgbox('error',getappdata(0,'hgui'),'错误','您选择的范围包含未分析的帧。','modal');
								selectionok=0;
								break
							end
						else
						...
						if size(ismean,1)>=strnum(i)
							if ismean(strnum(i))==1
								gui.custom_msgbox('error',getappdata(0,'hgui'),'错误','您的选择中不能包含已由平均矢量组成的帧。','modal');
								selectionok=0;
								break
							end
						else
							gui.custom_msgbox('error',getappdata(0,'hgui'),'错误','您选择的范围超过了已分析帧的数量。','modal');
							selectionok=0;
							break
						end
					end
				...
				gui.custom_msgbox('error',getappdata(0,'hgui'),'无法进行平均/求和...','一个会话中的所有分析必须具有相同尺寸，并且必须使用相同的 PIV 设置进行分析。','modal');
			end
		end
	end
end
