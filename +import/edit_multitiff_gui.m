...
video_loaded = 0;
%% Make figure
fig_handle = figure('MenuBar','none', 'Toolbar','none', 'Units','characters', 'WindowButtonDownFcn',@button_down, 'WindowButtonUpFcn',@button_up,  'WindowButtonMotionFcn', @on_click,'KeyPressFcn', @key_press,'Name','编辑多页 tiff','numbertitle','off','Visible','off','Windowstyle','modal','resize','on','dockcontrol','off');
...
handles.selectvideo = uicontrol(fig_handle,'Style','pushbutton','String','选择图像文件','Units','characters', 'Fontunits','points','Position',[item(1)+margin parentitem(4)-item(4)-margin-item(2) item(3)-margin*2 item(4)],'Callback', {@select_Callback, pathname},'Tag','selectvideo','TooltipString','选择图像文件');
item=[0 item(2)+item(4)+margin parentitem(3)/3*2 1];
handles.text1 = uicontrol(fig_handle,'Style','text','units', 'characters','Horizontalalignment', 'right','position',[item(1)+margin parentitem(4)-item(4)-margin-item(2) item(3)-margin*2 item(4)],'String','起始图层：');
...
handles.text2 = uicontrol(fig_handle,'Style','text','units', 'characters','Horizontalalignment', 'right','position',[item(1)+margin parentitem(4)-item(4)-margin-item(2) item(3)-margin*2 item(4)],'String','结束图层：');
item=[parentitem(3)/3*2 item(2) parentitem(3)/3*1 1];
...
item=[0 item(2)+item(4) parentitem(3)/3*2 1];
handles.text3 = uicontrol(fig_handle,'Style','text','units', 'characters','Horizontalalignment', 'right','position',[item(1)+margin parentitem(4)-item(4)-margin-item(2) item(3)-margin*2 item(4)],'String','每第 n 层处理：');
item=[parentitem(3)/3*2 item(2) parentitem(3)/3*1 1];
...
item=[0 item(2)+item(4)+margin parentitem(3)/3*2 1];
handles.text4 = uicontrol(fig_handle,'Style','text','units', 'characters','Horizontalalignment', 'right','position',[item(1)+margin parentitem(4)-item(4)-margin-item(2) item(3)-margin*2 item(4)],'String','预览图层编号：');
item=[parentitem(3)/3*2 item(2) parentitem(3)/3*1 1];
item=[0 item(2)+item(4)+margin parentitem(3) 2];
handles.importvideo = uicontrol(fig_handle,'Style','pushbutton','String','另存为...','Units','characters', 'Fontunits','points','Position',[item(1)+margin parentitem(4)-item(4)-margin-item(2) item(3)-margin*2 item(4)],'Callback', @import_Callback,'Tag','importvideo','TooltipString','保存修改后的 tiff 文件','enable','off');
	...
	function select_Callback(~,~,pathname)
		...
		if ~isequal(filename,0)
			...
			if success==1
                ...
                gui.custom_msgbox('error',getappdata(0,'hgui'),'错误',{'Matlab 无法导入此视频文件。最可能的原因是 Matlab 无法使用该视频编码解码器。这不是 PIVlab 的问题。确切的错误信息是： ' sprintf('\n') ME.identifier sprintf('\n') ME.message},'modal');
            end
        end
