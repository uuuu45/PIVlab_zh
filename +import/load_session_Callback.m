...
if valid_session_file==0
    gui.custom_msgbox('error',getappdata(0,'hgui'),'错误','这不是有效的 PIVlab 会话文件。','modal');
    display_hint=0;
end
gui.toolsavailable(1)

if display_hint==1 && gui.retr('batchModeActive') == 0
    gui.custom_msgbox('warn',getappdata(0,'hgui'),'警告','您加载了旧版 PIVlab 的会话。不建议这样做，可能会导致界面显示问题。','modal');
end
