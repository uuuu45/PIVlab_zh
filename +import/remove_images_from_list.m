...
if gui.retr('video_selection_done') ~= 1
	...
	if ~isempty(box_select) && numel(box_select)<=(numel(filepath)-2)
		...
		if mod(size(filepath,1),2)==1
			...
			gui.custom_msgbox('warn',getappdata(0,'hgui'),'警告：图像数量为奇数！',['有一幅图像无法配对。列表中的图像数量必须是偶数。' newline newline '请从列表中再移除一幅图像以继续。'],'modal');
		else
		...
		gui.custom_msgbox('error',getappdata(0,'hgui'),'错误：无法完成。','列表中至少保留两幅图像（一对）。','modal');
	end
else
    gui.custom_msgbox('error',getappdata(0,'hgui'),'错误：视频文件无法完成此操作','此操作仅适用于图像文件。','modal');
end