...
if ~isequal(path,0)
    ...
    if size(path,1) > 1 && multitiff == 0
        ...
        if sequencer==0 && proposed_sequencing==1 && ~batchModeActive
            ans_w = gui.custom_msgbox('quest',getappdata(0,'hgui'),'确定？',['检测到文件名以 "A" 和 "B" 结尾。这表明您应该使用"成对"序列方式，而不是"时间分辨"。' newline newline '需要我为您自动修正吗？'],'modal',{'是','否'},'是');
            if strcmp(ans_w,'是')
                sequencer=1;
    ...
    if pcopanda_dbl_image==1 && sequencer ~=1
        if ~batchModeActive
            gui.custom_msgbox('success',getappdata(0,'hgui'),'双图像多页 tiff 文件',['检测到 pco.panda 生成的双图像多页 tiff 文件。' newline newline '序列方式已改为"成对"，以处理双图像。'],'modal');
        end
        sequencer=1;
        gui.put('sequencer',sequencer);
        save('PIVlab_settings_default.mat','sequencer','-append');
    ...
    if loopcntr >= 1
        ...
        if size(framepart,1)>=2 %check if enough images loaded
            ...
            gui.displogo(0)
            gui.custom_msgbox('error',getappdata(0,'hgui'),'错误','选择必须至少包含两幅图像（= 1 对图像）','modal');
        end
    else
        gui.displogo(0)
        gui.custom_msgbox('error',getappdata(0,'hgui'),'错误','选择必须至少包含两幅图像（= 1 对图像）','modal');
