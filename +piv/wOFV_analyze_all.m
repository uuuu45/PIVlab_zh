...
if ok==1
    ...
    if strcmp(handles.ofv_median.String{handles.ofv_median.Value},'关闭')
        MedFiltFlag = false;
        MedFiltSize = [3,3];
    ...
    if strcmp(handles.ofv_parallelpatches.String{handles.ofv_parallelpatches.Value},'关闭')
    ...
    elseif strcmp(handles.ofv_parallelpatches.String{handles.ofv_parallelpatches.Value},'默认')
        tempImg = tempImg(roirect(2):roirect(2)+roirect(4)-1,roirect(1):roirect(1)+roirect(3)-1);
        PatchSize = 2^floor(log2(min(size(tempImg))));
        Fmats = wOFV.getFmatPyramid(PatchSize,PydLev);
        vartheta = ones(size(tempImg));
    ...
    for i=1:2:num_frames_to_process
        ...
        if isempty(cancel)==1 || cancel ~=1
            ...
            set(handles.progress, 'string' , ['帧进度：0%']);drawnow; %#ok<*NBRAK>
		...
		image2 = preproc.PIVlab_preproc( ...
            ...
            set(handles.overall, 'string' , ['总进度：' int2str((i+1)/2/num_frames_to_process*200) '%'])
            ...
            mins=floor(mins);
            secs=floor(secs);
            set(handles.totaltime,'string', ['剩余时间：' sprintf('%2.2d', hrs) 'h ' sprintf('%2.2d', mins) 'm ' sprintf('%2.2d', secs) 's']);
        end %cancel==0
    end

    delete(findobj('tag', 'annoyingthing'));
    set(handles.overall, 'string' , ['总进度：' int2str(100) '%'])
