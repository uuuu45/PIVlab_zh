...
if capturing==0
    ...
    if size(filepath,1)>0
        ...
        if gui.retr('video_selection_done') == 0
            set (handles.filenameshow, 'string', ['帧（' int2str(floor(get(handles.fileselector, 'value'))) '/' int2str(size(filepath,1)/2) '）：' sprintf('\n') filename{selected}]);
        ...
        if strncmp(get(handles.multip01, 'visible'), 'on',2)
            set(handles.imsize, 'string', ['图像尺寸：' int2str(size(currentimage,2)) '*' int2str(size(currentimage,1)) 'px' ])
        end
