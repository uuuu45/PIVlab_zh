...
button=gui.custom_msgbox('quest',getappdata(0,'hgui'),'同步器测试',['无线加密狗连接信息： '  driver_status ' ; ' access_status ' ; ' answer_status newline '激活同步器/激光测试（需要触发）？'],'modal',{'确定','取消'},'取消');
if strcmp(button,'确定')
    ...
    gui.custom_msgbox('quest',getappdata(0,'hgui'),'同步器测试','停止同步器/激光测试？','modal',{'确定'},'确定');
