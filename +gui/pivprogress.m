		...
		function tupdate(this)
			...
			if ishandle(this.hdl)
				try
					...
					if elapsd < 5
						remain_string='剩余时间：正在计算...';
					else
						remain_string=['剩余时间：' sprintf('%2.2d', hrs) 'h ' sprintf('%2.2d', mins) 'm ' sprintf('%2.2d', secs) 's'];
					end
					
					if this.percent ~= last_par_percent
						if ~isinf(remain_t)
							set(this.hdl, 'string' , ['总进度：' num2str(floor(100*this.percent)) '%' sprintf('\n') remain_string ]); %#ok<SPRINTFN>
