		...
		if exist ('bg_mode',"var") && exist ('bg_img_A',"var") && exist ('bg_img_B',"var") && ~isempty(bg_img_A) && ~isempty(bg_img_B) && (bg_mode == 2 || bg_mode==3)
			if piv_images_loaded
				...
				if (pivsize(1) ~= bgsize(1)) || (pivsize(2) ~= bgsize(2))
					gui.custom_msgbox("error",getappdata(0,'hgui'),'错误','The background image does not have the same size as your PIV images.','modal');
					return
				end
			end
			...
			gui.custom_msgbox("error",getappdata(0,'hgui'),'错误','This file is not containing valid background images. There must be three vars in the mat file: bg_mode, bg_img_A and bg_img_B. bg_mode must be 2 or 3, the other vars need to be image data with the same size as your PIV data.','modal');
		end
