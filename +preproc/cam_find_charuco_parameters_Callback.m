...
if ~isequal(filename,0)
    ...
    if detectionOK
        ...
        gui.custom_msgbox('msg',getappdata(0,'hgui'),'QR 码结果',['原点棋盘格颜色：' originCheckerColor newline '标记族：' markerFamily newline '行数：' int2str(patternDims(1)) newline '列数：' int2str(patternDims(2)) newline '棋盘格尺寸：' int2str(checkerSize) newline '标记尺寸：' int2str(markerSize)],'modal','OK','OK');
        return
	else
    ...
    if isempty(imagePoints)
        gui.custom_msgbox('error',getappdata(0,'hgui'),'错误','无法估算参数','modal');
        return
    end
    ...
    gui.custom_msgbox('msg',getappdata(0,'hgui'),'结果（可能不准确）',['原点棋盘格颜色：' OriginCheckerColor  '标记族：' mostCommonFamily  '行数：' int2str(boardSize(1))  '列数：' int2str(boardSize(2)) ],'modal','OK','OK');
