...
if startframe ~=endframe && gui.retr('displaywhat')>1 && get(handles.autoscaler,'Value') == 1 %user wants to export multiple frames and displays derivatives, and hasn't enabled a fixed colormap range
    answer = gui.custom_msgbox('quest',getappdata(0,'hgui'),'颜色图范围未固定','您尚未为颜色图设置固定范围。这可能会导致颜色图闪烁。请为颜色图选择固定范围（禁用自动缩放）。仍然继续？ ','modal',{'是','否'},'否');
	if strcmp(answer , '否')
        continue_export=0;
...
if continue_export==1
    [filename, pathname] = uiputfile(formatstring, '保存图像为',fullfile(imgsavepath, 'PIVlab_out'));
else
