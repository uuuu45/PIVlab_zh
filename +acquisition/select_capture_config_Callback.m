...
if strcmpi(config_string,'PIVlab LD-PS + OPTRONIS Cyclone') % OPTRONIS
	...
	switch camera_sub_type
		...
		case 'Cyclone-25-150-M-bitflow'
			...
			gui.put('blind_time',25);
		otherwise
			gui.custom_msgbox('error',getappdata(0,'hgui'),'未找到相机',{'未找到相机。相机是否已连接并通电？' '是否已安装"Image Acquisition Toolbox Support Package for GenICam Interface"或 BitFlow MATLAB IMAQ 适配器？'},'modal');
...
if strcmpi(config_string,'Webcam demo (no synchronizer)')
	...
	if webcam_err~=1
        ...
        if ~verLessThan('matlab','25')
            ...
            uialert(getappdata(0,'hgui'),message,'未找到网络摄像头','icon','warning','Interpreter','html');
        else
            disp('https://www.mathworks.com/matlabcentral/fileexchange/45182-matlab-support-package-for-usb-webcams')
            gui.custom_msgbox('error',getappdata(0,'hgui'),'未找到网络摄像头','无法访问网络摄像头。此功能需要免费的 MATLAB USB 网络摄像头支持包附加组件（链接现已显示在命令窗口中）。','modal');
        end
