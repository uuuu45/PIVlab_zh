...
if strcmp(camera_type,'pco_pixelfly') || strcmp(camera_type,'chronos')  %ROI selection not yet available for pixelfly and chronos
    gui.custom_msgbox('error',getappdata(0,'hgui'),'不可用','所选相机类型尚不支持 ROI 选择。','modal');
end

if strcmp(camera_type,'flir')
    gui.custom_msgbox('error',getappdata(0,'hgui'),'不可用','FLIR 相机系列的 ROI 选择功能即将实现！','modal');
end
...
if strcmp(camera_type,'OPTRONIS')
    ...
    if verLessThan('matlab','25') && ~endsWith(camera_sub_type_ver, '-bitflow')
        gui.custom_msgbox('error',getappdata(0,'hgui'),'不可用','OPTRONIS 的 ROI 选择只能在 Matlab 版本 > R2025a 时进行。','modal');
    ...
    if capture_ok==1
        ...
        if strcmp(camera_type,'pco_panda') ||strcmp(camera_type,'pco_edge26')
            try
                ...
                disp(ME)
                gui.custom_msgbox('error',getappdata(0,'hgui'),'相机未连接','相机未连接','modal');
                gui.displogo
                capture_ok=0;
            end
