...
if verLessThan('matlab','9.7') %R2019b
	gui.custom_msgbox('error',getappdata(0,'hgui'),'需要更新的 Matlab','PIVlab 中的图像采集和同步器控制至少需要 MATLAB 9.7 (R2019b) 版本。', 'modal');
    return
end
handles=gui.gethand;
...
if alreadyconnected
    ...
    set(handles.ac_connect,'String','连接');
    ...
    if isempty(serports)
        serports='未找到可用的串口！';
        set(handles.ac_connect,'String','刷新');
    else
        set(handles.ac_connect,'String','连接');
    end
...
if gui.retr('parallel')==1
	button = gui.custom_msgbox('quest',getappdata(0,'hgui'),'关闭并行池？','强烈建议在图像采集期间关闭并行处理以节省内存。','modal',{'确定','取消'},'确定');
	if strncmp(button,'确定',2)==1
