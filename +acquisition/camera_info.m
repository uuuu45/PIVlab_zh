...
try
    ...
    gui.custom_msgbox('warn',getappdata(0,'hgui'),'可用设备',['未找到相机。' newline newline '请按照以下网址中的说明，确认驱动程序和附加组件的正确安装：' newline newline 'https://www.pivlab.de/wiki/5-camera-setup/'],'modal');
...
if isempty(hwinfo.DeviceIDs) && isempty(cam_name)
    ...
    gui.custom_msgbox('warn',getappdata(0,'hgui'),'可用设备',['检测到的相机：' newline deviceListStr],'modal');
end
gui.toolsavailable(1)
