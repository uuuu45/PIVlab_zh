...
if isdeployed
    splashscreen = figure('integerhandle','off','resize','off','windowstyle','modal','numbertitle','off','MenuBar','none','DockControls','off','Name','PIVlab 独立版','Toolbar','none','Units','pixels','Position',[10 10 150 150],'tag','splashscreen','visible','off','handlevisibility','on');
    ...
    handle_splash_text = text(splash_ax,300,590,'正在生成图形窗口，请稍候...','Color','w','VerticalAlignment','bottom','HorizontalAlignment','center','FontName','FixedWidth','Fontweight','bold','FontSize',9,'Interpreter','none');
    drawnow expose
end
%% Make figure
fh = findobj('tag', 'hgui');
if isempty(fh)
    disp('-> 正在启动 PIVlab...')
    MainWindow = figure('numbertitle','off','MenuBar','none','Windowstyle','normal','DockControls','off','Name','正在初始化...','Toolbar','none','Units','normalized','Position',[0 0.1 1 0.8],'tag','hgui','visible','off');
        ...
        try %checking for a parallel license file throws a huge error message wheh it is not available. This might scare users... Better: Try...catch block
            if ~exist('desired_num_cores','var') %no input argument --> use all existing cores
                if misc.pivparpool('size')<=0 %no exisitng pool
                    ...
                    figure(MainWindow)
                    pause(0.05)
                    answer=gui.custom_msgbox('quest',getappdata(0,'hgui'),'是否打开并行池？',['PIVlab 可以使用并行计算运行。' newline newline '- 处理多幅图像时推荐使用。' newline '- 采集图像或处理 mp4/avi 文件时不需要。' newline newline '是否打开并行池？'],'modal',{'是' '否'},'否');
                    pause(0.1)
                    switch answer
                        case '是'
